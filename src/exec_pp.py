...
from net_trainer import ClassificationModel
...
def main(args):
    ...
    results_dir = os.path.join(output_dir, f"fold_{fold}_{seed}", postprocessor_name, f"{experiment_name}_{id_database}_{ood_database}_version_{version}")
    ...
    if "resnet18" in experiment_name:
    ...
    elif "efficientnet_l" in experiment_name:
        batch_size = 32
    ...
    elif "vit_b_16" in experiment_name:
    ...
    # Load model
    if epoch!=None:
        model_path = os.path.join("models", f"{experiment_name}_{id_database}_fold_{fold}_{seed}", f"version_{version}", "checkpoints", f"{experiment_name}_{id_database}_fold_{fold}_{seed}_epoch={epoch}-val_loss=0.00.ckpt")
    else:
        checkpoints_dir = os.path.join("models", f"{experiment_name}_{id_database}_fold_{fold}_{seed}", f"version_{version}", "checkpoints")
    ...
    id_path = os.path.join("folds", id_database)
    ood_path = os.path.join("folds", ood_database)

    if "super_cifar100" in id_database:
        id_data = torch.load(f"{id_path}_{seed}/test_ID_fold_{fold}.pt")
        ood_data = torch.load(f"{ood_path}_{seed}/test_OOD_fold_{fold}.pt")
        train_data = torch.load(f"{id_path}_{seed}/train_ID_fold_{fold}.pt")
    else:
        id_data = torch.load(f"{id_path}_{seed}/test_strat_fold_{fold}.pt")
        ood_data = torch.load(f"{ood_path}_{seed}/test_group_fold_{fold}.pt")
        train_data = torch.load(f"{id_path}_{seed}/train_strat_fold_{fold}.pt")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', required=True, type=str, help='Seed')
