...
class MDSPostprocessor(BasePostprocessor):
    ...
    def setup_emb(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        if not self.setup_flag:
            ...
            all_feats = id_loader_dict['train']["embeddings"].cpu()
            all_labels = deepcopy(id_loader_dict['train']["labels"])
            all_labels = all_labels.squeeze()
            all_preds = id_loader_dict['train']["logits"].argmax(1).cpu()
            # sanity check on train acc
            train_acc = all_preds.eq(all_labels).float().mean()
            print(f' Train acc: {train_acc:.2%}')
            ...
            self.class_mean = torch.stack(self.class_mean).float()  # shape [#classes, feature dim]
    ...
    def postprocess_emb(self, net: nn.Module, data: Any):
        logits, features = data["logits"].cpu(), data["embeddings"].cpu()
        pred = logits.argmax(1)
        ...
        for c in range(self.num_classes):
            tensor = features.cpu() - self.class_mean[c].view(1, -1)
            class_scores[:, c] = -torch.matmul(
                torch.matmul(tensor.float(), self.precision.float()), tensor.t().float()).diag().float()
