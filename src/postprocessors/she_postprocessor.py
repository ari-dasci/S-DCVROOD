...
class SHEPostprocessor(BasePostprocessor):
    ...
    def setup_emb(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        if not self.setup_flag:
            ...
            all_activation_log = id_loader_dict['train']["embeddings"].data.cpu()
            ...
            self.activation_log = torch.cat(self.activation_log).cpu()
            self.setup_flag = True
        else:
            pass
    ...
    def postprocess_emb(self, net: nn.Module, data: Any):
        output, feature = data["logits"].cpu(), data["embeddings"].cpu()
        pred = output.argmax(1)
        conf = distance(feature, self.activation_log[pred], self.args.metric)
        return pred, conf