...
class fDBDPostprocessor(BasePostprocessor):
    ...
    def setup_emb(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        if not self.setup_flag:
            ...
            activation_log_concat = id_loader_dict['train']['embeddings'].data.cpu().float()
            self.activation_log = activation_log_concat
            self.train_mean = activation_log_concat.mean(dim=0).cpu()
            ...
            self.denominator_matrix = torch.tensor(denominator_matrix).float()
            self.setup_flag = True
