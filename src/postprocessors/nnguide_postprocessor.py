...
def knn_score(bankfeas, queryfeas, k=100, min=False):
    ...
    bankfeas = np.ascontiguousarray(deepcopy(np.array(bankfeas))).astype(np.float32)
    queryfeas = np.ascontiguousarray(deepcopy(np.array(queryfeas))).astype(np.float32)
...
class NNGuidePostprocessor(BasePostprocessor):
    ...
    def setup_emb(self, net: nn.Module, id_loader_dict, ood_loader_dict):
        if not self.setup_flag:
            ...
            bank_feas = id_loader_dict['train']["embeddings"][:int(
                               len(id_loader_dict['train']["logits"]) *
                               self.alpha)].data.float().cpu().numpy()
            bank_confs = logsumexp(id_loader_dict['train']["logits"][:int(
                               len(id_loader_dict['train']["logits"]) *
                               self.alpha)].data.float().cpu().numpy(),
                                   axis=-1)
            self.bank_guide = bank_feas * bank_confs[:, None]
    ...
    def postprocess_emb(self, net: nn.Module, data: Any):
        logit, feature = data["logits"], data["embeddings"]
        feas_norm = normalizer(feature.data.cpu().numpy())
        energy = logsumexp(logit.data.cpu().numpy(), axis=-1)
