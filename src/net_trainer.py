...
class ClassificationModel(pl.LightningModule):
    def __init__(self, base_model, num_classes, hyperparameters, classes_weights=None):
    ...
    def training_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        # Retrieve data
        images, labels_ids = batch

        # Compute classification loss
        logits = self(images)
        if dataloader_idx == 0:  # ID Data: Standard Classification Loss
            ...
            self.log("train/outlier_loss", loss)

        del images, labels_ids  
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if dataloader_idx == 0:  # ID Data: Standard Classification Loss
            loss = self.criterion(logits, labels_ids)
            labels_ids_preds = torch.argmax(logits, dim=1)

            # Compute metrics
            self._val_accuracy.update(labels_ids_preds, labels_ids)
            self._val_loss.update(loss)

            # Log metrics
            self.log_dict({'val/loss': self._val_loss, 'val/accuracy': self._val_accuracy}, on_step=False, on_epoch=True)
        
        else:
            if self.method == "softmax":  # Basic OE (Uniform KL-Div Loss)
                uniform_target = torch.ones_like(logits) / logits.size(1)
                self.val_outlier_loss = F.kl_div(F.log_softmax(logits, dim=1), uniform_target, reduction="batchmean")
            elif self.method == "energy":  # EnergyOE (Softplus on Energy Score)
                energy = self.energy_score(logits)
                self.val_outlier_loss = torch.mean(F.softplus(energy))

            self.log_dict({'val/outlier_loss': self.val_outlier_loss, 'val/accuracy': self._val_accuracy}, on_step=False, on_epoch=True)

            

        # Forcefully free the batch memory after its use
        del images, labels_ids  
        torch.cuda.empty_cache()
    ...
    def transfer_batch_to_device(self, batch, device, dataloader_idx=None):
        """ Move ONLY the current batch to GPU, keeping others on CPU """
        images, labels = batch  # Unpack the batch (assuming images and labels)
        images, labels = images.to(device), labels.to(device)
        
        return images, labels # Move both to GPU

    def get_fc(self):
        return self.classifier[-1]

def obtain_data(X,y, hyperparameters, transform):
...
def main(args):
    ...
    if version is not None:
        ...
        name_of_file = name_of_file + "_" + version
        last_checkpoint_path = os.path.join(checkpoints_dir, checkpoints_names[-1])
    ...
    if outliers is not None:
        if not apply_ood:
            outliers_data = torch.load(f"{data_path}/train_OOD_fold_{fold}.pt")
        else:  
            outliers_data = torch.load(f"{data_path}/train_OOD_fold_{fold}.pt")

    # Define transformations (Normalization for CIFAR-10) 
    ...
    if outliers is not None:
        X_outlier = [img for img, _ in outliers_data]
        y_outlier = [label for _, label in outliers_data]
        train_loader_outlier,val_loader_outlier,_ = obtain_data(X_outlier,y_outlier,hyperparameters, transform)
    ...
    if outliers is not None:
        trainer.fit(model, [train_loader, train_loader_outlier], [val_loader, val_loader_outlier], ckpt_path=last_checkpoint_path)
    else:
