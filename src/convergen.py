...
postprocesoss=["ebo"]
iddatabases=["cifar100"]
ooddatabases=["cifar10"]
versions=["version_1"]
...
def spearman_multiple_shuffles(metric_list, num_shuffles=100):
    rho_values = []
    p_values = []
    for _ in range(num_shuffles):
        ...
        # Calcular Spearman
        rho, p_value = spearmanr(x, metric_list)
        rho_values.append(np.abs(rho))
        p_values.append(p_value)
    
    # Promedio de rho y p-value de todas las barajas
    avg_rho = np.mean(rho_values)
    avg_p_value = np.mean(p_values)
    
    return avg_rho, avg_p_value

def mini_test(metric_name ,data):
    variance = np.std(data) / np.mean(data)
    avg_rho, avg_p_value = spearman_multiple_shuffles(data, 100)
    print(f"{metric_name}, Variance test: {variance}")
    print(f"{metric_name}, Spearman test, rho: {avg_rho}, p_value: {avg_p_value}")
...
def main():
            ...
            for i in range(len(iddatabases)):
                for postprocessor_name in postprocesoss:
                    ...
                    for fold in folds:
                        ...
                        output_path = os.path.join("gt_results", f"fold_{fold}", postprocessor_name, f"{experiment_name}_{iddatabases[i]}_{ooddatabases[i]}_{version}")
                        
                        # print(f"{fold}_{postprocessor_name}_{experiment_name}_{iddatabases[i]}_{ooddatabases[i]}_{version}" , flush=True)
    ...
    sumary_extended_pd.to_excel("gt_results/summary_extended.xlsx", index=False)
    sumary_pd.to_excel("gt_results/summary.xlsx", index=False)

    sumary_extended_pd.to_csv("gt_results/summary_extended.csv", index=False)
    sumary_pd.to_csv("gt_results/summary.csv", index=False)

    resultado = sumary_extended_pd.groupby(["experiments", "versions", "postprocess", "id_database", "ood_database"]).apply(test).reset_index()

    print("Variance")

    print(f"Auroc: {np.std(auroc_extended_list) / np.mean(auroc_extended_list)}", flush=True)
    print(f"Aupur: {np.std(aupr_extended_list) / np.mean(aupr_extended_list)}", flush=True)
    print(f"f1: {np.std(f1_extended_list) / np.mean(f1_extended_list)}", flush=True)
    print(f"tpr: {np.std(tpr_at_fpr_extended_list) / np.mean(tpr_at_fpr_extended_list)}", flush=True)
    print(f"fpr{np.std(fpr_at_tpr_extended_list) / np.mean(fpr_at_tpr_extended_list)}", flush=True)
    print(f"acc_99: {np.std(accuracy_99_extended_list) / np.mean(accuracy_99_extended_list)}", flush=True)
    print(f"acc_95: {np.std(accuracy_95_extended_list) / np.mean(accuracy_95_extended_list)}", flush=True)

    # Aplicar la función a cada métrica
    print("Spearman (Promedio de múltiples barajas):")

    # Auroc
    avg_rho, avg_p_value = spearman_multiple_shuffles(auroc_extended_list, 100)
    print(f"Auroc, rho: {avg_rho}, p_value: {avg_p_value}")

    # Aupur
    avg_rho, avg_p_value = spearman_multiple_shuffles(aupr_extended_list, 100)
    print(f"Aupur, rho: {avg_rho}, p_value: {avg_p_value}")

    # F1
    avg_rho, avg_p_value = spearman_multiple_shuffles(f1_extended_list, 100)
    print(f"f1, rho: {avg_rho}, p_value: {avg_p_value}")

    # TPR
    avg_rho, avg_p_value = spearman_multiple_shuffles(tpr_at_fpr_extended_list, 100)
    print(f"tpr, rho: {avg_rho}, p_value: {avg_p_value}")

    # FPR
    avg_rho, avg_p_value = spearman_multiple_shuffles(fpr_at_tpr_extended_list, 100)
    print(f"fpr, rho: {avg_rho}, p_value: {avg_p_value}")

    # Accuracy 99
    avg_rho, avg_p_value = spearman_multiple_shuffles(accuracy_99_extended_list, 100)
    print(f"acc_99, rho: {avg_rho}, p_value: {avg_p_value}")

    # Accuracy 95
    avg_rho, avg_p_value = spearman_multiple_shuffles(accuracy_95_extended_list, 100)
    print(f"acc_95, rho: {avg_rho}, p_value: {avg_p_value}")
